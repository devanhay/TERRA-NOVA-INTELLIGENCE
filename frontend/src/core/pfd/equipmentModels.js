...
export function solveReactor(p, inlet, compA, compB, modelType, presetKey) {
  ...
  const dHrxn_kJmol = parseFloat(p.rxnHeat) || 0   // kJ/mol A reacted
  ...
  const Q_rxn = n_A_reacted * dHrxn_kJmol

  const cpL_out = cpLiqMix(zA_out, zB_out, compA, compB)
  const dT_adiabatic = -Q_rxn / (massFlow_out * cpL_out)
...
export function solveFlash(p, inlet, isAutoSeparator, compA, compB, modelType, presetKey) {
  ...
  if (flash.psi <= 0.001) {
    return { ok: false, reason: `All-liquid at ${T.toFixed(1)}°C / ${P.toFixed(0)} kPa. No vapor phase produced. Raise T or lower P.` }
  }
  if (flash.psi >= 0.999) {
    return { ok: false, reason: `All-vapor at ${T.toFixed(1)}°C / ${P.toFixed(0)} kPa. No liquid phase produced. Lower T or raise P.` }
  }

  const vaporMass = flow * flash.psi
  const liquidMass = flow * (1 - flash.psi)
  ...
  const h_out = specificEnthalpy(T, flash.psi, flash.xA, flash.xB, flash.yA, flash.yB, compA, compB)
...
export function solveRadFracColumn(p, inlet, compA, compB, modelType, presetKey) {
  ...
  // Settle vapor-liquid internal profiles
  const L = new Array(N).fill(R * D)
  const V = new Array(N).fill((R + 1) * D)

  // Adjust for feed stage
  ...
  for (let iter = 0; iter < 40; iter++) {
    for (let j = 0; j < N; j++) {
      const kv = kValues(T[j], P_col, xA[j], 1 - xA[j], compA, compB, modelType, presetKey, yA[j], 1 - yA[j])
      const KA = kv.KA
      ...
      if (j === 0) {
        ...
        // Reboiler stage (bottom)
        xA[j] = (L[j - 1] * xA[j - 1]) / (V[j] * KA + B)
      } else if (j === Nf) {
        // Feed stage
        const L_in = L[j - 1]
        const V_in = V[j + 1]
...
export function solveDistillation(p, inlet, compA, compB, modelType, presetKey) {
  ...
  const B = F - D
  if (D <= 0 || B <= 0) return { ok: false, reason: `Separation targets impossible: D=${D.toFixed(2)} kg/s, B=${B.toFixed(2)} kg/s.` }

  // Relative volatility at feed temperature
  const alpha_F = relVol(inlet.temp, compA, compB)
  if (alpha_F <= 1.0) return { ok: false, reason: `Relative volatility α=${alpha_F.toFixed(3)} ≤ 1. Distillation impossible.` }
  ...
  const dHvapMix_D = wA_D * dHvapA_cond + (1 - wA_D) * dHvapB_cond
  const Qc = (R + 1) * D * dHvapMix_D
  const Qr = Qc + D * inlet.enthalpy - B * inlet.enthalpy

  // Outlet stream temperatures
  ...
  const warnings = []
  if (N > 100) warnings.push(`Very high stage count (N=${N}).`)
  if (R > 5 * Rmin) warnings.push(`R/Rmin = ${(R/Rmin).toFixed(2)} >> 1. Over-refluxing.`)
...
export function solveValve(p, inlet, compA, compB, modelType, presetKey) {
  ...
  // Iterate to find T_out such that h(T_out, P_out) = h_in
  for (let iter = 0; iter < 40; iter++) {
  ...
  if (flash_out && flash_out.psi > 0.01) {
    warnings.push(`Flashing across valve: ${(flash_out.psi * 100).toFixed(1)}% of stream vaporizes.`)
  }
...
export function solvePRV(p, inlet, compA, compB, modelType, presetKey) {
  ...
  const Pb = parseFloat(p.backPressure) || 101.3  // kPa
  
  if (isNaN(P_set) || P_set <= 0) return { ok: false, reason: 'Set pressure must be > 0 kPa' }

  const { flow, temp, press, zA, zB } = inlet
  const isPopped = press > P_set * 1.10
...
export function solveCompressor(p, inlet, compA, compB, modelType, presetKey) {
  ...
  if (inlet.psi < 0.9) return { ok: false, reason: `Compressor requires vapor feed. Use pump for liquids.` }
  ...
  const warnings = []
  if (T_out > 250) warnings.push(`High discharge temperature: ${T_out.toFixed(1)} °C.`)
  if (ratio > 10) warnings.push(`Very high compression ratio (${ratio}).`)
...
export function solveStorageTank(p, inlet) {
  ...
  const fillRate = inlet.flow / (inlet.density || 900)
  const fillTime = vol / fillRate / 3600
...
export function solveElectrolyzer(p, inlet, compA, compB, modelType, presetKey) {
  ...
  const T = parseFloat(p.temp) || 80
  const P = parseFloat(p.press) || 3000
  ...
  if (molSplit >= molA_in * 0.99) {
    warnings.push("Feed water starvation!")
  }
