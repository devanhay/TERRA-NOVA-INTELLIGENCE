...
export function gammaGas(zA, zB, compA, compB) {
  ...
  const Cv = cpV - R_GAS / (MW * 1000) * 1000  // kJ/(kg·K) — R_GAS in J/mol, MW in g/mol
  // Cv = Cp - R/MW  where R=8.314 J/mol·K, MW in g/mol → R/MW in J/g·K = kJ/kg·K
...
export function solvePRCompressibility(T_C, P_kPa, zA, zB, compA, compB, phase = 'Vapor') {
  ...
  if (D > 0) {
    ...
    const T = Math.sign(R_val - sqrtD) * Math.pow(Math.abs(R_val - sqrtD), 1/3)
    roots.push(-a1 / 3 + (S + T))
  } else {
